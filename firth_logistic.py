    ...
    for p in range(y_all.shape[1]):
        ...
        if covars is not None:
            ...
            X = X_orig - (
                covars
                @ (
                    np.linalg.inv((covars.T * y_pred_null) @ covars)
                    @ (covars.T @ (X_orig.T * y_pred_null).T)
                )
            )
        # Initialize weights
        ...
        if iter == max_iter - 1:
            ...
            se_all[p] = se
    return weights_all, se_all, loglike_all, iters_all
...
if __name__ == "__main__":
    ...
    beta, se, loglike_snp, iters = firth_logit_svt(geno, y_all, pred_covars, covar)
    lr_stat = 2 * (loglike_snp - loglike_null)
